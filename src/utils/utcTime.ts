...
/**
 * Get local datetime 1 hour from now for validation (social/weather feeds).
 */
export function getMinimumDateTimeOneHour(): Date {
...
/**
 * Check if a date is at least 1 hour in the future.
 */
export function isValidFutureTimeOneHour(date: Date): boolean {
  return date.getTime() >= getMinimumDateTimeOneHour().getTime();
}
