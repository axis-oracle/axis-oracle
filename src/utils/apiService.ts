...
// Supported chains for memecoin module
export const MEMECOIN_CHAINS = [
  { id: 'solana', name: 'Solana', shortName: 'SOL' },
  { id: 'eth', name: 'Ethereum', shortName: 'ETH' },
  { id: 'base', name: 'Base', shortName: 'BASE' },
  { id: 'bsc', name: 'BNB Chain', shortName: 'BNB' },
] as const;

export type MemecoinChainId = typeof MEMECOIN_CHAINS[number]['id'];
...
export function isValidAddressForChain(address: string, chainId: MemecoinChainId): boolean {
  if (chainId === 'solana') {
...
// Fetch memecoin data from GeckoTerminal (supports multiple chains)
export async function fetchMemeData(tokenAddress: string, chainId: MemecoinChainId = 'solana'): Promise<MemeTokenData | null> {
  try {
    const response = await fetch(
      `https://api.geckoterminal.com/api/v2/networks/${chainId}/tokens/${tokenAddress}`,
      {
        headers: {
    ...
    const chain = MEMECOIN_CHAINS.find(c => c.id === chainId);
    
    // Try to get holders count from relationships if available
    let holdersCount: number | undefined;
    if (data.data?.relationships?.top_pools?.data?.[0]) {
      // GeckoTerminal doesn't provide holders directly, we'll use a placeholder
      // In production, you'd need to call a separate API for holder counts
...
export function getGeckoTerminalUrl(module: string, config: Record<string, unknown>): { url: string; jsonPath: string } {
  if (module === 'memecoin') {
    ...
    const chainId = String(config.chain || 'solana');
    const metric = String(config.metric || 'price').toLowerCase();
    const isMcap = metric.includes('cap') || metric === 'marketcap';
    
    return {
      url: `https://api.geckoterminal.com/api/v2/networks/${chainId}/tokens/${contractAddress}`,
  ...
  if (module === 'crypto') {
    ...
    if (mapping) {
      const jsonPath = metric === 'volume' ? '$.data.attributes.volume_usd.h24' : '$.data.attributes.price_usd';
      return {
