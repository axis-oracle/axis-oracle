...
const getModuleIcon = (feed: Feed): { iconSrc: string; alt: string; isDynamic: boolean } => {
  ...
  switch (feed.module) {
    ...
    case 'sports':
      return { iconSrc: sportsIcon, alt: 'Sports', isDynamic: false };
    case 'social':
      return { iconSrc: xIcon, alt: 'Social', isDynamic: false };
    case 'launchpad':
      // Use token logo if available
      const launchpadTokenImage = config?.tokenImage as string;
      if (launchpadTokenImage) {
        return { iconSrc: launchpadTokenImage, alt: (config?.tokenSymbol as string) || 'Token', isDynamic: true };
      }
      return { iconSrc: launchpadIcon, alt: 'Launchpad', isDynamic: false };
    case 'movies':
      // Use poster if available
      const posterPath = config?.posterPath as string;
      if (posterPath) {
        return { iconSrc: `https://image.tmdb.org/t/p/w200${posterPath}`, alt: (config?.movieTitle as string) || 'Movie', isDynamic: true };
      }
      return { iconSrc: moviesIcon, alt: 'Movies', isDynamic: false };
    case 'politics':
      return { iconSrc: politicsIcon, alt: 'Politics', isDynamic: false };
    case 'economy':
      return { iconSrc: economyIcon, alt: 'Economy', isDynamic: false };
    case 'awards':
      return { iconSrc: awardsIcon, alt: 'Awards', isDynamic: false };
    case 'legal':
      return { iconSrc: legalIcon, alt: 'Legal', isDynamic: false };
    case 'custom':
      return { iconSrc: aiIcon, alt: 'AI Prediction', isDynamic: false };
    default:
...
const getShortTitle = (feed: Feed): string => {
  ...
  switch (feed.module) {
    ...
    case 'sports':
      ...
      return feed.title.split(' Winner')[0] || 'Match';
    case 'social':
      const tweetAuthor = config.tweetAuthor as string;
      return tweetAuthor ? `@${tweetAuthor}` : 'Social Market';
    case 'movies':
      return (config.movieTitle as string) || 'Movie';
    default:
...
const getMetricLabel = (feed: Feed): string => {
  ...
  switch (feed.module) {
    ...
    case 'social':
      ...
      if (metric === 'retweets') return 'Retweets';
      return 'Social Metric';
    case 'movies':
      if (metric === 'vote_average') return 'Rating';
      if (metric === 'popularity') return 'Popularity';
      if (metric === 'revenue') return 'Box Office';
      return 'Movie Metric';
    default:
      return metric || 'Value';
  }
...
const getGameBadge = (feed: Feed): { label: string; className: string } | null => {
  ...
  if (feed.module === 'sports') {
    const league = config.league as string;
    const sport = config.sport as string;
    
    // Sport-specific colors
    switch (sport) {
      case 'basketball':
        return { label: league || 'NBA', className: 'bg-orange-500/20 text-orange-400 border-orange-500/30' };
      case 'hockey':
        return { label: league || 'NHL', className: 'bg-blue-500/20 text-blue-400 border-blue-500/30' };
      case 'soccer':
        return { label: league || 'Soccer', className: 'bg-green-500/20 text-green-400 border-green-500/30' };
      case 'tennis':
        return { label: league || 'Tennis', className: 'bg-purple-500/20 text-purple-400 border-purple-500/30' };
      default:
        return { label: league || 'Sports', className: 'bg-secondary text-muted-foreground border-border' };
    }
  }
  
  if (feed.module === 'social') {
    return { label: 'X/Twitter', className: 'bg-sky-500/20 text-sky-400 border-sky-500/30' };
  }
  
  if (feed.module === 'movies') {
    return { label: 'TMDB', className: 'bg-green-500/20 text-green-400 border-green-500/30' };
  }
...
const formatSettledValue = (value: string, config: any, module: string): string => {
  ...
  // Sports: show winner team name
  if (module === 'sports') {
    const winnerValue = Math.round(numValue);
    if (winnerValue === 1) {
      return `Winner: ${config?.homeTeam || 'Home'}`;
    } else if (winnerValue === 2) {
      return `Winner: ${config?.awayTeam || 'Away'}`;
    } else {
      return 'Draw';
    }
  }
  
  // Social: format large numbers (likes, retweets)
  if (module === 'social') {
    if (numValue >= 1e6) {
  ...
  // Movies: format rating, popularity, or revenue
  if (module === 'movies') {
  ...
  // Volume: format as currency with compact notation
  if (metric === 'volume') {
    if (numValue >= 1e9) {
  ...
  // Holders: format as integer with compact notation
  if (metric === 'holders') {
    if (numValue >= 1e6) {
      return `${(numValue / 1e6).toFixed(1)}M holders`;
    } else if (numValue >= 1e3) {
      return `${(numValue / 1e3).toFixed(1)}K holders`;
    }
    return `${formatNumber(Math.round(numValue))} holders`;
  }
