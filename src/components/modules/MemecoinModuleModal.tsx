...
import { fetchMemeData, formatPrice, formatMarketCap, MemeTokenData, MEMECOIN_CHAINS, MemecoinChainId, isValidAddressForChain } from '@/utils/apiService';
...
type MetricType = 'price' | 'marketcap' | 'holders' | 'volume';

const MIN_MARKET_CAP = 100_000;

interface MemecoinModuleModalProps {
  open: boolean;
...
export const MemecoinModuleModal: FC<MemecoinModuleModalProps> = ({ 
  open, 
  onOpenChange, 
  onCreateFeed, 
  isLoading, 
  ...
  const [selectedChain, setSelectedChain] = useState<MemecoinChainId>('solana');
  ...
  const isValidAddress = isValidAddressForChain(contractAddress, selectedChain);
  ...
  useEffect(() => {
    const fetchData = async () => {
      ...
      try {
        const data = await fetchMemeData(contractAddress, selectedChain);
        if (data) {
          setTokenData(data);
          setFetchError(false);
        } else {
  ...
  }, [contractAddress, selectedChain, isValidAddress]);
  ...
  const autoGeneratedTitle = useMemo(() => {
    ...
    const metricLabel = metric === 'price' ? 'Price' : metric === 'marketcap' ? 'Market Cap' : metric === 'volume' ? '24h Volume' : 'Holders';
  ...
  const currentChain = MEMECOIN_CHAINS.find(c => c.id === selectedChain);
      ...
      <DialogContent className="sm:max-w-[480px] bg-card border-border">
        <DialogHeader>
          <div className="flex items-center gap-3">
            <img src={pepeIcon} alt="Token Analytics" className="h-10 w-10 object-contain" />
            <div>
              <DialogTitle className="text-xl font-bold">Token Analytics</DialogTitle>
              <p className="text-sm text-muted-foreground">Multi-chain tracking for memecoins & tokens</p>
            </div>
          </div>
        </DialogHeader>
        ...
        <div className="space-y-4 mt-4">
          ...
          <div className="space-y-2">
            ...
            <Select value={selectedChain} onValueChange={(value) => {
              setSelectedChain(value as MemecoinChainId);
              setTokenData(null);
              setFetchError(false);
            }}>
              ...
              <SelectContent>
                {MEMECOIN_CHAINS.map((chain) => (
                  <SelectItem key={chain.id} value={chain.id}>
                    <span className="flex items-center gap-2">
                      <span className="font-medium">{chain.name}</span>
                      <span className="text-muted-foreground text-xs">({chain.shortName})</span>
                    </span>
                  </SelectItem>
                ))}
              </SelectContent>
          ...
          <div className="space-y-2">
            <div className="flex items-center gap-2">
              ...
              <TooltipProvider>
                ...
                <TooltipContent className="max-w-[250px]">
                  <p>Only tokens with &gt;$100K Market Cap are allowed to ensure liquidity and accurate data.</p>
                </TooltipContent>
                </Tooltip>
              </TooltipProvider>
            </div>
            <Input
              placeholder={selectedChain === 'solana' ? "Paste Solana address..." : "Paste 0x address..."}
              ...
              <div className="flex items-start gap-3 p-3 bg-destructive/10 border border-destructive/20 rounded-md">
                ...
                <div className="flex-1 min-w-0">
                  ...
                  <p className="text-xs text-destructive mt-2">
                    Market Cap is below $100K. Too risky for an oracle.
                  </p>
                </div>
              </div>
            )}
            ...
            {fetchError && !isFetching && isValidAddress && (
              <div className="flex items-center gap-2 p-2 bg-destructive/10 border border-destructive/20 rounded-md">
                ...
                <span className="text-sm text-destructive">Token not found on {currentChain?.name || 'selected chain'}</span>
              </div>
            )}
          ...
          <div className="space-y-2">
            ...
            <div className="flex bg-muted rounded-lg p-1">
              <button
                ...
                className={cn(
                  "flex-1 py-2 px-1.5 text-xs font-medium rounded-md transition-all duration-200",
                  metric === 'price'
