...
type CryptoMetricType = 'price' | 'volume';

interface CryptoModuleModalProps {
  open: boolean;
  onOpenChange: (open: boolean) => void;
  onCreateFeed: (config: { 
    symbol: string; 
    quoteCurrency: 'USD';
    metric: CryptoMetricType;
    resolutionDate: Date;
    title: string;
  }) => void;
  isLoading?: boolean;
...
export const CryptoModuleModal: FC<CryptoModuleModalProps> = ({ 
  open, 
  onOpenChange, 
  onCreateFeed, 
  isLoading, 
  ...
  const [metric, setMetric] = useState<CryptoMetricType>('price');
  ...
  const autoGeneratedTitle = useMemo(() => {
    ...
    const metricLabel = metric === 'price' ? 'Price' : '24h Volume';
    return `${selectedAsset.symbol}/USD ${metricLabel} [${dateLabel}]`;
  }, [selectedAsset, fullResolutionDate, metric]);
      ...
      <DialogContent className="sm:max-w-[480px] bg-card border-border">
        <DialogHeader>
          <div className="flex items-center gap-3">
            <img src={bitcoinIcon} alt="Global Crypto" className="h-10 w-10 object-contain" />
            <div>
              <DialogTitle className="text-xl font-bold">Global Crypto</DialogTitle>
              <p className="text-sm text-muted-foreground">Streaming high-precision data for global markets</p>
            </div>
          </div>
        </DialogHeader>
        ...
        <div className="space-y-4 mt-4">
          ...
          <div className="space-y-2">
            ...
            <p className="text-xs text-muted-foreground">Select your local time — it will be converted to UTC automatically.</p>
          </div>
