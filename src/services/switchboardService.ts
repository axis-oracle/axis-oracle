...
const CREATION_FEE_LAMPORTS = CREATION_FEE_SOL * LAMPORTS_PER_SOL;
...
export async function deployOracle(
  ...
  try {
    ...
    const tx = await sb.asV0Tx({
      connection,
      ixs: [initIx], // Only Switchboard init instruction, no platform fee for now
      payer: wallet.publicKey,
    ...
    if (error instanceof Error) {
      ...
      if (error.message.includes('insufficient funds') || error.message.includes('Insufficient') || error.message.includes('0x1')) {
        return { success: false, error: 'Insufficient SOL balance. You need at least 0.05 SOL.' };
      }
...
export function buildOracleJobDefinition(module: string, config: Record<string, unknown>): { tasks: object[] } {
  switch (module) {
    ...
    case 'memecoin': {
      // MEMECOINS (GeckoTerminal API - supports multiple chains)
    ...
    case 'sports': {
      // SPORTS (The Odds API - scores endpoint)
      const sportKey = String(config.sportKey || '');
      ...
      return {
        tasks: [
          {
            httpTask: {
              url: `https://api.the-odds-api.com/v4/sports/${sportKey}/scores?apiKey=${config.oddsApiKey}&eventIds=${eventId}`,
            },
          },
          {
            jsonParseTask: {
              // The API returns array, we extract completed status and scores
              path: '$[0].completed',
            },
          },
        ],
