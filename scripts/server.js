...
app.post('/settle-feed', requireApiKey, async (req, res) => {
  ...
  const { feedPubkey, feedHash, feedId, module, config, preComputedValue, skipFetchUpdate } = req.body;
  
  console.log(`Parsed - feedPubkey: ${feedPubkey}, feedHash: ${feedHash}, feedId: ${feedId}, skipFetchUpdate: ${skipFetchUpdate}, preComputedValue: ${preComputedValue}`);
  
  if (!feedPubkey) {
    return res.status(400).json({ success: false, error: 'feedPubkey is required' });
  }
  
  // feedHash not required for memo transactions (Sports/Esports with preComputedValue)
  if (!feedHash && !skipFetchUpdate) {
    return res.status(400).json({ success: false, error: 'feedHash is required for on-chain settlement (unless skipFetchUpdate is true)' });
  }
  
  console.log(`Single feed settlement requested: ${feedPubkey} with hash: ${feedHash}`);
  
  try {
    const result = await settlerModule.settleSingleFeed({ feedPubkey, feedHash, feedId, module, config, preComputedValue, skipFetchUpdate });
    res.json({ 
      success: true, 
