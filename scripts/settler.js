...
// Memo Program ID (standard Solana program for on-chain notes)
...
/**
 * Create a memo transaction as on-chain proof for Sports/Esports
 * This records the settlement result without using Switchboard oracles
 */
async function createMemoTransaction(feedPubkey, preComputedValue, connection, settler) {
  console.log(`Creating memo transaction for feed ${feedPubkey} with value ${preComputedValue}`);
  
  // Create memo instruction with settlement data
  const memoData = `AXIS:${feedPubkey}:${preComputedValue}`;
  ...
  console.log(`Memo transaction sent: ${signature}`);
  ...
  if (confirmation.value.err) {
    throw new Error(`Memo transaction failed: ${JSON.stringify(confirmation.value.err)}`);
  }
  
  console.log('Memo transaction confirmed!');
  
  return { signature, settledValue: preComputedValue };
}

/**
 * Settle a single feed on-chain using Switchboard SDK v3
 * For Sports/Esports with preComputedValue, uses memo transaction instead
 */
async function settleFeedOnChain(feed, connection, settler, program) {
  ...
  const preComputedValue = feed.preComputedValue;
  const skipFetchUpdate = feed.skipFetchUpdate;
  
  console.log(`Feed pubkey: ${feedPubkey}`);
  console.log(`Feed hash: ${feedHash}`);
  console.log(`Module: ${feed.module}`);
  console.log(`Pre-computed value: ${preComputedValue}, Skip fetch: ${skipFetchUpdate}`);
  
  // For Sports/Esports with pre-computed value, use memo transaction
  if (skipFetchUpdate && preComputedValue !== undefined) {
    console.log(`Using memo transaction for ${feed.module} module`);
    return await createMemoTransaction(feedPubkey, preComputedValue, connection, settler);
  }
  
  // Standard path for Crypto/Memecoin/Weather - use Switchboard oracles
  console.log('Using Switchboard fetchUpdateIx for oracle data...');
    ...
    if ((url.pathname === '/settle' || url.pathname === '/settle-feed') && req.method === 'POST') {
      ...
      req.on('end', async () => {
        try {
          ...
          const { feedPubkey, feedHash, feedId, module, config, preComputedValue, skipFetchUpdate } = parsed;
          
          console.log(`Parsed - feedPubkey: ${feedPubkey}, feedHash: ${feedHash}, feedId: ${feedId}`);
          console.log(`preComputedValue: ${preComputedValue}, skipFetchUpdate: ${skipFetchUpdate}`);
          ...
          // feedHash not required for memo transactions (sports/esports)
          if (!feedHash && !skipFetchUpdate) {
            res.writeHead(400, { 'Content-Type': 'application/json' });
          ...
          console.log(`Feed hash: ${feedHash}, Module: ${module}`);
          
          const { connection, program, settler } = await initializeSwitchboard();
          
          const feed = { feedPubkey, feedHash, id: feedId, module, config, preComputedValue, skipFetchUpdate };
          const { signature, settledValue } = await settleFeedOnChain(
            feed, connection, settler, program
          );
 ...
 */
async function settleSingleFeed({ feedPubkey, feedId, module, config, feedHash, preComputedValue, skipFetchUpdate }) {
  const { connection, program, settler } = await initializeSwitchboard();
